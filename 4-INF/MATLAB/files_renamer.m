...
Nchar = 1;                                  % number of characters to be cnaceled in REM or KEP mode
...
for j = 1:Nfiles
    ...
    if strcmp(Mod,'ADD_I')                                      % add "Str" string at the title beginning
        newname = strcat(Str,oldname,extension);
    elseif strcmp(Mod,'ADD_F')                                  % add "Str" string at the title end
        newname = strcat(oldname,Str,extension);
    elseif strcmp(Mod,'REM_I')                                  % remove "Nchar" characters at the title beginning
        newname = strcat(oldname(Nchar+1:end),extension);
    elseif strcmp(Mod,'REM_F')                                  % remove "Nchar" characters at the title end
        newname = strcat(oldname(1:end-Nchar),extension);
    elseif strcmp(Mod,'KEP_I')                                  % keep "Nchar" characters at the title beginning (and remove all the rest)
        newname = strcat(oldname(1:Nchar),extension);
    elseif strcmp(Mod,'KEP_F')                                  % keep "Nchar" characters at the title end (and remove all the rest)
