...
def create_fleet( args ) :
    ...
    al_ynum = get_aliens_ynum(args['params'],args['ship'].rect.height,alien.rect.height)
...
def update_aliens( args ) :
    ...
    args['aliens'].update(args['params'].alien_speedup**args['stats'].speedup_cnt)
...
def ship_hit( args ) :
    ...
    if args['stats'].ships_left > 0 :
        ...
        args['stats'].game_state = 'INACTIVE'                               # assert game-over flag if no lives left
        pygame.mouse.set_visible(True)                                      # make mouse visible again when game is inactive
