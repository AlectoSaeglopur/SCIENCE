...
BaseType_t xReturn;
	...
	#else
	{
		...
		xReturn = xTaskCreate(	prvIdleTask,                                    /** @vkn: Create idle task (bind to function "prvIdleTask") */
	...
	if( xReturn == pdPASS )
	{
		...
		portable interface. */
		if( xPortStartScheduler() != pdFALSE )                                  /** @vkn: Tick timer initialized here! */
		{
...
// @vfi: added from web library (to create static task)
