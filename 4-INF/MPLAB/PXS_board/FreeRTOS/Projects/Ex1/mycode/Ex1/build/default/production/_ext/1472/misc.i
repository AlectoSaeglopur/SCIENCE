...
static void Blink_LED0( void *pvParameters ){
    ...
    Config_GPIO((uint8_t[2]) {(uint8_t) 1,15},Par);
    Lock_GPIOs();
    Set_GPIO((uint8_t[2]) {(uint8_t) 1,15},(uint8_t) 1);
    while( 1 ){
        vTaskDelay(((TickType_t)(((TickType_t)((uint16_t) 51)))*(((TickType_t)( ( TickType_t ) 1000 ))/( TickType_t)1000)));
        Toggle_GPIO((uint8_t[2]) {(uint8_t) 1,15});
    }
}
...
static void Blink_LED1( void *pvParameters ){
    ...
    Lock_GPIOs();
    Set_GPIO(GpioId,(uint8_t) 1);
    TickType_t xLastWakeTime = xTaskGetTickCount();
    while( 1 ){
        vTaskDelayUntil(&xLastWakeTime,((TickType_t)(((TickType_t)((uint16_t) 9)))*(((TickType_t)( ( TickType_t ) 1000 ))/( TickType_t)1000)));
        Toggle_GPIO(GpioId);
    }
}
...
void ConfigMod_Stats( void ){
    ...
    if( Hdl_Timer == 0 ){
        Set_ErrFlag((uint16_t) 0x0404);
    } else {
        xTimerGenericCommand( ( Hdl_Timer ), ( ( BaseType_t ) 1 ), ( xTaskGetTickCount() ), 0, ( 0 ) );
    }
}
...
static void Print_Stats( TimerHandle_t xTimer ){
    ...
    uint16_t TasksNumber = uxTaskGetNumberOfTasks();
    TaskStatus_t TasksStatus[TasksNumber];
    TasksNumber = uxTaskGetSystemState(
            TasksStatus,TasksNumber,&TotalRunTime);
    TotalRunTime /= 10000;
    if( (TotalRunTime > 0) ){

        Add_Printf_str("NAME  ",( 8 ));
        Add_Printf_str(" | ",3);
        ...
        Start_Printf();
        { __delay32( (unsigned long) (((unsigned long long) (uint8_t) 2)*((uint32_t) 90000000)/1000ULL)); };
        Add_Printf_str("--------------------------------------",38);
        Start_Printf();
        { __delay32( (unsigned long) (((unsigned long long) (uint8_t) 2)*((uint32_t) 90000000)/1000ULL)); };

        for( j=0; j<TasksNumber; j++ ){
            Add_Printf_str(TasksStatus[j].pcTaskName,( 8 ));
            Add_Printf_str(" | ",3);
            Add_Printf_udec8(TasksStatus[j].eCurrentState);
            Add_Printf_str(" | ",3);
            Add_Printf_udec8(TasksStatus[j].uxCurrentPriority);
            Add_Printf_str(" | ",3);
            Add_Printf_udec16(TasksStatus[j].usStackHighWaterMark);
            Add_Printf_str(" | ",3);
        ...
        Add_Printf_udec16(xPortGetFreeHeapSize());
        Add_Printf_str(" B\r\n",4);
        Start_Printf();
    } else {
...
void ConfigMod_Filter( void ){
    ...
    if( Hdl_Queue == 0 ){
        ...
        Ret = xTaskCreate( Update_DAC, "DAC3", 6*( uint16_t ) 20,
                0, (uint8_t) 5,0 );
        if( Ret == ( -1 ) ){
            Set_ErrFlag((uint16_t) 0x0401);
        }
...
void __attribute__((interrupt, auto_psv)) _ADCAN3Interrupt( void ){
    const uint16_t AdcVal6 = ADCBUF3;
    IFS5bits.ADCAN3IF = 0;
...
static void Kick_Watchdog( void *pvParameters ){
    ...
    TickType_t xLastWakeTime = xTaskGetTickCount();
    for( ;; ){
        vTaskDelayUntil(&xLastWakeTime,((TickType_t)(((TickType_t)((uint16_t) 3500)))*(((TickType_t)( ( TickType_t ) 1000 ))/( TickType_t)1000)));
