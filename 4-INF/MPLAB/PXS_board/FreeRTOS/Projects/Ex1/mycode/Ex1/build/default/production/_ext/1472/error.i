...
static uint8_t ErrRegs[(uint8_t) 20] = {0};
# 36 "../error.c"
void Set_ErrFlag( uint16_t ErrId ){
    uint8_t Idx = (ErrId&0xFF00u)>>8;
    uint8_t Mask = (uint8_t)(ErrId&0x00FFu);
    if( Idx < (uint8_t) 20 ){
        ErrRegs[Idx] |= Mask;
    }
}
...
void Reset_ErrRegs( void ){
    void *Ret = memset(ErrRegs,0,(uint8_t) 20);
    if( Ret != ErrRegs ){
       Set_ErrFlag((uint16_t) 0x0101);
    }
}
# 100 "../error.c"
void __attribute__((interrupt, auto_psv)) _OscillatorFail(void) {
    if( INTCON1bits.OSCFAIL == 1 ){
        Set_ErrFlag((uint16_t) 0x0201);
    }
    if( OSCCONbits.CF == 1 ){
        Set_ErrFlag((uint16_t) 0x0202);
    }
    if( OSCCONbits.LOCK == 1 ){
        Set_ErrFlag((uint16_t) 0x0204);
    }
...
void __attribute__((interrupt, auto_psv)) _MathError(void) {
    if( INTCON1bits.OVAERR == 1 ){
        Set_ErrFlag((uint16_t) 0x0240);
    }
    if( INTCON1bits.OVBERR == 1 ){
        Set_ErrFlag((uint16_t) 0x0280);
    }
    if( INTCON1bits.COVAERR == 1 ){
        Set_ErrFlag((uint16_t) 0x0301);
    }
    if( INTCON1bits.COVBERR == 1 ){
        Set_ErrFlag((uint16_t) 0x0302);
    }
    if( INTCON1bits.SFTACERR == 1 ){
        Set_ErrFlag((uint16_t) 0x0304);
    }
    if( INTCON1bits.DIV0ERR == 1 ){
        Set_ErrFlag((uint16_t) 0x0308);
    }
    if( INTCON1bits.MATHERR == 1){
        Set_ErrFlag((uint16_t) 0x0220);
    }
...
void __attribute__((interrupt, auto_psv)) _SoftTrapError( void ){
    if(INTCON3bits.DAE == 1 ){
        Set_ErrFlag((uint16_t) 0x0320);
    }
    if(INTCON3bits.DOOVR == 1){
        Set_ErrFlag((uint16_t) 0x0310);
    }
    INTCON3bits.DAE = 0;
    INTCON3bits.DOOVR = 0;
}
