...
void vStartBlockingQueueTasks( UBaseType_t uxPriority )
{
    ...
    const TickType_t xBlockTime = pdMS_TO_TICKS( ( TickType_t ) 1000 );     // @vfi: convert 1s into corresponding number of ticks
    const TickType_t xDontBlock = ( TickType_t ) 0;

	/* Create the first two tasks as described at the top of the file. */

	/* 1. First create the structure used to pass parameters to the consumer tasks. */
	pxQueueParameters1 = ( xBlockingQueueParameters * ) pvPortMalloc( sizeof( xBlockingQueueParameters ) );     // @vfi : allocate space for the 1st struct (buffer not included, since allocated below)
	/* Create the queue used by the first two tasks to pass the incrementing number.
	Pass a pointer to the queue in the parameter structure. */
	pxQueueParameters1->xQueue = xQueueCreate( uxQueueSize1, ( UBaseType_t ) sizeof( uint16_t ) );
	/* The consumer is created first so gets a block time as described above. */
	pxQueueParameters1->xBlockTime = xBlockTime;                                            // @vfi : set block time to 1s
	/* Pass in the variable that this task is going to increment so we can check it
	is still running. */
	pxQueueParameters1->psCheckVariable = &( sBlockingConsumerCount[ 0 ] );

	/* 2. Create the structure used to pass parameters to the producer task. */
	pxQueueParameters2 = ( xBlockingQueueParameters * ) pvPortMalloc( sizeof( xBlockingQueueParameters ) );
	/* Pass the queue to this task also, using the parameter structure. */
	pxQueueParameters2->xQueue = pxQueueParameters1->xQueue;                                // @vfi : link producer to consumer queue
	...
	/* Note the producer has a lower priority than the consumer when the tasks are created. */
	...
	xTaskCreate( vBlockingQueueProducer, "QProdB2", blckqSTACK_SIZE, ( void * ) pxQueueParameters2, tskIDLE_PRIORITY, NULL );
     // @vfi: create new task specifying type (e.g. consumer queue), name (arbitrarily), stack size, queue struct and priority

    
	/* 3-4. Create the second two tasks as described at the top of the file.   This uses
	...
	/* 5-6. Create the last two tasks as described above.  The mechanism is again just
