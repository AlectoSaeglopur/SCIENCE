MWS Result File Version 20070618
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
balance
1D Results\Balance\Balance [1]
^1.bil

Frequency / GHz
S-Parameter Balance
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
energy
1D Results\Energy\Energy [1]
^1.eng

Time / ns
Field Energy / dB
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
complex
1D Results\Power\Excitation [1]\Power Accepted per Port\Port 1
^AcceptedPower_1(1).sig
W
Frequency / GHz
Power in Watt
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
complex
1D Results\Power\Excitation [1]\Power Accepted
^AcceptedPower_1.sig
W
Frequency / GHz
Power in Watt
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
complex
1D Results\Efficiencies\Rad. Efficiency [1]
^FarfieldMetaData_1_RadEff.sig

Frequency / GHz
Radiation Efficiency
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
complex
1D Results\Efficiencies\Tot. Efficiency [1]
^FarfieldMetaData_1_TotEff.sig

Frequency / GHz
Total Efficiency
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
complex
1D Results\Power\Excitation [1]\Power Radiated
^RadiatedPower_1.sig
W
Frequency / GHz
Power in Watt
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
complex
1D Results\Power\Excitation [1]\Power Outgoing all Ports
^ReflectedPower_1.sig
W
Frequency / GHz
Power in Watt
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
complex
1D Results\Power\Excitation [1]\Power Stimulated
^StimulatedPower_1.sig
W
Frequency / GHz
Power in Watt
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
complex
1D Results\Reference Impedance\ZRef 1(1)
^ZRef1(1).sig

Frequency / GHz
Impedance in Ohm
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
complex
1D Results\Power\Excitation [1]\Loss per Material\Metal loss in Copper (annealed)
^cMetal_loss_Copper (annealed)(1).sig
W
Frequency / GHz
Power in Watt
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
complex
1D Results\S-Parameters\S1,1
^cS1(1)1(1).sig

Frequency / GHz
S-Parameter
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
complex
1D Results\Power\Excitation [1]\Loss in Metals
^cTotal_metal_loss(1).sig
W
Frequency / GHz
Power in Watt
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
time
1D Results\Port signals\i1
^i1(1).sig

Time / ns
Time Signals
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
time
1D Results\Port signals\o1,1
^o1(1)1(1).sig

Time / ns
Time Signals
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
complex
1D Results\Discrete Ports\Currents\Port 1 [1]
^pic1(1).sig

Frequency / GHz
Discrete Port Current
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
time
1D Results\Discrete Ports\Currents\Signals\Port 1 [1]
^pis1(1).sig

Time / ns
Discrete Port Current Time Signals
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
complex
1D Results\Discrete Ports\Voltages\Port 1 [1]
^puc1(1).sig

Frequency / GHz
Discrete Port Voltage
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
time
1D Results\Discrete Ports\Voltages\Signals\Port 1 [1]
^pus1(1).sig

Time / ns
Discrete Port Voltage Time Signals
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
complex
1D Results\Discrete Ports\Impedances\Port 1 [1]
^pzc1(1).sig

Frequency / GHz
Discrete Port Impedance
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
linear
1D Results\VSWR\VSWR1
^vswr1(1).sig

Frequency / GHz
Voltage Standing Wave Ratio (VSWR)
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
complex
1D Results\Y Matrix\Y1,1
^ycmplx1(1)1(1).sig

Frequency / GHz
A/V Matrix Coefficients in Y
:8:1000
visible
internal
surviveparchange
result
PARAMETRIC_1D
complex
1D Results\Z Matrix\Z1,1
^zcmplx1(1)1(1).sig

Frequency / GHz
V/A Matrix Coefficients in Z
:8:1000
visible
internal
survivemeshadapt
result
HIDDENITEM
^PP.fmm
:8:1000
visible
internal
survivemeshadapt
result
HIDDENITEM
^model.gex
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
time
1D Results\Discrete Ports\Voltages\Signals\Port 1 [1]
^pus1(1).sig

Time / ns
Discrete Port Voltage Time Signals
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
time
1D Results\Discrete Ports\Currents\Signals\Port 1 [1]
^pis1(1).sig

Time / ns
Discrete Port Current Time Signals
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
complex
1D Results\Discrete Ports\Voltages\Port 1 [1]
^puc1(1).sig

Frequency / GHz
Discrete Port Voltage
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
complex
1D Results\Discrete Ports\Currents\Port 1 [1]
^pic1(1).sig

Frequency / GHz
Discrete Port Current
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
complex
1D Results\Discrete Ports\Impedances\Port 1 [1]
^pzc1(1).sig

Frequency / GHz
Discrete Port Impedance
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^pua1(1).sig
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^pia1(1).sig
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^pud1(1).sig
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^pid1(1).sig
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^pup1(1).sig
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^pip1(1).sig
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
complex
1D Results\Power\Excitation [1]\Loss per Material\Metal loss in Copper (annealed)
^cMetal_loss_Copper (annealed)(1).sig
W
Frequency / GHz
Power in Watt
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^aMetal_loss_Copper (annealed)(1).sig
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^dMetal_loss_Copper (annealed)(1).sig
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
complex
1D Results\Power\Excitation [1]\Loss in Metals
^cTotal_metal_loss(1).sig
W
Frequency / GHz
Power in Watt
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^aTotal_metal_loss(1).sig
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^dTotal_metal_loss(1).sig
:8:1000
visible
internal
rebuild
result
HFIELD3D
2D/3D Results\H-Field\h-field (f=0.8) [1]
^h-field (f=0.8)_1,1.m3d
h-field (f=0.8) [1]
:8:1000
visible
internal
rebuild
result
SURFACECURRENT
2D/3D Results\Surface Current\surface current (f=0.8) [1]
^h-field (f=0.8)_1,1.m3d
surface current (f=0.8) [1]
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
time
1D Results\Port signals\i1
^i1(1).sig

Time / ns
Time Signals
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
time
1D Results\Port signals\o1,1
^o1(1)1(1).sig

Time / ns
Time Signals
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
complex
1D Results\Power\Excitation [1]\Power Stimulated
^StimulatedPower_1.sig
W
Frequency / GHz
Power in Watt
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
complex
1D Results\Power\Excitation [1]\Power Outgoing all Ports
^ReflectedPower_1.sig
W
Frequency / GHz
Power in Watt
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
complex
1D Results\Power\Excitation [1]\Power Accepted
^AcceptedPower_1.sig
W
Frequency / GHz
Power in Watt
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
complex
1D Results\Power\Excitation [1]\Power Accepted per Port\Port 1
^AcceptedPower_1(1).sig
W
Frequency / GHz
Power in Watt
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
energy
1D Results\Energy\Energy [1]
^1.eng

Time / ns
Field Energy / dB
:8:1000
visible
internal
rebuild
result
FARFIELD
Farfields\farfield (f=0.8) [1]
^farfield (f=0.8)_1.ffm|
farfield (f=0.8) [1]
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
complex
1D Results\S-Parameters\S1,1
^cS1(1)1(1).sig

Frequency / GHz
S-Parameter
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^a1(1)1(1).sig
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^d1(1)1(1).sig
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^p1(1)1(1).sig
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
complex
1D Results\Y Matrix\Y1,1
^ycmplx1(1)1(1).sig

Frequency / GHz
A/V Matrix Coefficients in Y
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^yam1(1)1(1).sig
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^yim1(1)1(1).sig
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^yre1(1)1(1).sig
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^yph1(1)1(1).sig
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
complex
1D Results\Z Matrix\Z1,1
^zcmplx1(1)1(1).sig

Frequency / GHz
V/A Matrix Coefficients in Z
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^zam1(1)1(1).sig
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^zim1(1)1(1).sig
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^zre1(1)1(1).sig
:8:1000
visible
internal
rebuild
result
HIDDENITEM
^zph1(1)1(1).sig
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
linear
1D Results\VSWR\VSWR1
^vswr1(1).sig

Frequency / GHz
Voltage Standing Wave Ratio (VSWR)
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
balance
1D Results\Balance\Balance [1]
^1.bil

Frequency / GHz
S-Parameter Balance
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
complex
1D Results\Power\Excitation [1]\Power Radiated
^RadiatedPower_1.sig
W
Frequency / GHz
Power in Watt
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
complex
1D Results\Efficiencies\Rad. Efficiency [1]
^FarfieldMetaData_1_RadEff.sig

Frequency / GHz
Radiation Efficiency
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
complex
1D Results\Efficiencies\Tot. Efficiency [1]
^FarfieldMetaData_1_TotEff.sig

Frequency / GHz
Total Efficiency
:8:1000
visible
internal
rebuild
result
XYSIGNAL2
complex
1D Results\Reference Impedance\ZRef 1(1)
^ZRef1(1).sig

Frequency / GHz
Impedance in Ohm
Low Frequency:4:3
visible
internal
persistent
noresult
XYSIGNAL2
user
Excitation Signals\default
^signal_default_lf.sig

Time / ns
Excitation: default
High Frequency:0:0
visible
internal
persistent
noresult
XYSIGNAL2
user
Excitation Signals\default
^signal_default.sig

Time / ns
Excitation: default
