...
const uint32_t Fc = 10e6;												// Carrier frequency [Hz] (NB: Max 4.3 GHz with uint32 representation)
const double h = 0.5;													// Modulation index
...
const double beta = 0.5;												// 3-dB bandwidth - symbol time (BT) product
...
const double dev = h*Rs/2;                                           	// Inner frequency deviation [Hz]
const uint32_t FsBB = osf*Rs;                                          	// Ideal baseband sample rate [Sa/s]
const double TsBB = 1.0/FsBB;                                          	// Ideal baseband sample period [s]
...
void FreqMapper( uint8_t *InBytes, double *OutSymbs, freqmap *Table );
void FskModulatorBB( double *InSymbs, complex *OutSamps );
void Upsampling( double *InStream, double *OutStream, uint32_t OutLen, uint8_t UpFactor, uint8_t Mode );
void LinResamplingR( double *InStream, double *OutStream, uint32_t InLen, uint32_t OutLen, uint32_t FsIn, uint32_t FsOut );
void LinResamplingC( complex *InStream, complex *OutStream, uint32_t InLen, uint32_t OutLen, uint32_t FsIn, uint32_t FsOut );
void IntergatorR( double *InSamps, double *OutSamps, uint32_t SampLen, double Ts );
void IntergatorC( complex *InSamps, complex *OutSamps, uint32_t SampLen, double Ts );
void CpfskModulatorBB( double *InSymbs, complex *OutSamps );
void GetGaussTaps( double *Taps, double BT, uint8_t span, uint8_t sps );
void ScaleGaussTaps( double *Taps, uint16_t Ntaps );
void FirFilterR( double *InSamps, double *OutSamps, double *Taps, uint16_t Ntaps );
void GfskModulatorBB( double *InSymbs, complex *OutSamps );

void RandGen( uint8_t *IoBytes );
void GenUpktSitael( uint8_t *IoBytes );
void PrintParam( void );
void PrintBytes( uint8_t *Bytes, uint16_t Len );
void PrintSymbs( double *Symbs, uint16_t Len );
...
void CheckParam( void ){
	...
	if ( log2((double)M) != round(log2((double)M)) ){
...
void FskModulatorBB( double *InSymbs, complex *OutSamps ){
	...
	uint32_t i;
	double Time;															// Sampling instant
	double FreqShift[SampLen];												// Temporary sample stream representing frequency shift in time
...
/**
 * Function for upsammpling the incoming double-stream by the specified input factor
...
void LinResamplingR( double *InStream, double *OutStream, uint32_t InLen, uint32_t OutLen, uint32_t FsIn, uint32_t FsOut ){
	...
	uint32_t j = 1;
	double m, q, TimeOut;
	double TsIn = 1.0/FsIn;													// Original sample period
	double TsOut = 1.0/FsOut;												// Target sample period
	double TimeAxisIn[InLen];
...
void LinResamplingC( complex *InStream, complex *OutStream, uint32_t InLen, uint32_t OutLen, uint32_t FsIn, uint32_t FsOut ){
	...
	uint32_t j = 1;
	double m, q, OutTime;
	double TsIn = 1.0/FsIn;
	double TsOut = 1.0/FsOut;
	double InTimeAxis[InLen];
...
void CpfskModulatorBB( double *InSymbs, complex *OutSamps ){
	...
	uint32_t i;
	double FrPhShift[SampLen];											// Temporary sample stream representing frequency and phase shift in time
 ...
 **/
void GetGaussTaps( double *Taps, double beta, uint8_t span, uint8_t sps ){

	uint16_t j;
	double Time;
	double alpha = sqrt(log(2)/2)/beta;
	uint16_t Ntaps = span*sps+1;

	for ( j=0; j<Ntaps; j++ ){
		Time = (j-(double)sps*span/2)/sps;
...
void GfskModulatorBB( double *InSymbs, complex *OutSamps ){
	...
	double FreqPhShift[SampLen];										// Temporary sample stream representing frequency/phase shift in time
	double GaussTaps[Ntaps];											// Gaussian taps for FIR filtering
...
void WriteSamplesCsv( complex *InSamps, uint32_t Len, uint32_t SampRate, uint8_t Label ){
	...
	if (fid != NULL){
		...
		fprintf(fid,"%1.2f",(double)SampRate);
	}
	fclose(fid);
