...
\newcommand{\myparvspace}{\vspace{4mm}}   % vertical space between paragraphs
\newcommand{\mysecvspace}{\vspace{6mm}}   % vertical space between sections
...
\newcommand{\myexfont}{\small}   % font size for example code or shell text
...
\item[\mybulletlvA] By default the \textit{git stash [push]} command stores:
\begin{itemize}
...
\item[\mybulletlvA] Note it is even possible to stash only specific modified files instead of them all. To do so, use \mycmd{git stash push <files-or-directories> -m "<message>"} (e.g. \textit{git stash push Source/Application -m "WIP on ticket 471"}).
\myparvspace
...
\begin{itemize}
  ...
  \item[] \textcolor{\mytextfilecolor}{\textit{\myexfont \;\;\;\; 1 file changed, 2 insertions(+)}}
  \item[]
  \item[] \textcolor{\mytextfilecolor}{\textit{\myexfont ValmoFil@zBookG10 MINGW64 /c/git\_projects/ETO\_SWFW\_Projects/FPP\_Peacock\_HUI ((9e3405b4...)|BISECTING)}}
