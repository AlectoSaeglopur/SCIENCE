...
\begin{itemize}
  \item[] \textit{<{}<{}<{}<{}<{}<{}<}
  \item[] \textit{... section \#1 ... \: $\rightarrow$\: destination branch / HEAD}
  \item[] \textit{======= \;\;\;\;\;\;\:\:\: $\rightarrow$\: conflict divider}
  \item[] \textit{... section \#2 ... \: $\rightarrow$\: source branch}
  \item[] \textit{>{}>{}>{}>{}>{}>{}>}
\end{itemize}
\myvspace

\item[$\circ$] To check which project files contain conflicts to be solved after a merge use \mycmd{git diff -{}-check} (if none or after solving all of them, the command simply returns no message). Plus, they are labeled as \textit{unmerged} as a result of \textit{git status}. Another way is to simply search for the string \textcolor{teal}{\textit{<{}<{}<{}<{}<{}<{}< HEAD}} identifying any pending conflict throughout the project. Note conflicts can arise also due to \textit{git stash pop|apply} commands (see Section \mysecref{sec:Stash}); in this case look for the string \textcolor{teal}{\textit{>{}>{}>{}>{}>{}>{}> Stashed changes}} instead.
\end{itemize}
\myvspace
...
% label section 15 as "Rebase" + "Stash" !!!
