...
const float Peb = 0.004;													// Bit error probability for BSC simulation
...
const uint8_t PrimPolyGF4[] = {0,1,0,0};									// Primitive polynomial for GF(2^4) (maximum degree is implied)
const uint8_t PrimPolyGF8[] = {0,2,3,4};									// Primitive polynomial for GF(2^8) (maximum degree is implied)
...
void ChanBSC( uint8_t *OutBytes, uint8_t *InBytes, float Peb, uint16_t (*ChanErr)[2] );
uint16_t CheckFinalErr( uint8_t *FinErr, uint8_t *SrcBytes, uint8_t *DecBytes );
...
void ChanBSC( uint8_t *OutBytes, uint8_t *InBytes, float Peb, uint16_t (*ChanErr)[2] ){
	...
	uint8_t BitIdx;
	uint8_t Mask = 0x01;
	uint16_t BitErrCount = 0;
	uint16_t SymbErrCount = 0;
	uint8_t MsgIdx = 0;
	bool LockSymbErr = false;
	if ( (InBytes != NULL) && (OutBytes != NULL) ){
		memcpy(OutBytes,InBytes,EncLen);
		BitLen = (EncLen<<3);
		for ( j=0; j<BitLen; j++ ){
...
/**
 *	Function for estimating the final number of mismatched bits after the decoding.
 **/
uint16_t CheckFinalErr( uint8_t *FinErr, uint8_t *SrcBytes, uint8_t *DecBytes ){
	uint8_t BitIdx;
	uint16_t j, ByteIdx;
	uint16_t ErrCount = 0;														// Bit error counter within current message
	uint16_t ErrTot = 0;														// Overall number of currupted bits among all messages
	uint8_t MsgIdx = 0;
	uint8_t Mask = 0x01;
	uint16_t BitLen = (UncLen<<3);
	if ( (SrcBytes != NULL) && (DecBytes != NULL) && (FinErr != NULL) ){
		for ( j=0; j<BitLen; j++){
			ByteIdx = (j>>3);
			BitIdx = (uint8_t)(j&0x0007);
			if ( (((SrcBytes[ByteIdx])>>BitIdx)&Mask) != (((DecBytes[ByteIdx])>>BitIdx)&Mask) ){
				ErrCount++;
			}
			if ( (j%(m*k)) == (m*k-1) ){
				FinErr[MsgIdx] = ErrCount;
				ErrTot += ErrCount;
				ErrCount = 0;
				MsgIdx++;
			}
