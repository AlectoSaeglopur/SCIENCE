...
const uint16_t ByteLen = 6000;											// Source info stream length (in bytes)
const double EbN0 = 8;													// [dB] Energy-per-bit-to-noise-power-spectral-density ratio over AWGN channel (NB: SNR=Eb/N0*log2(M)/sps)
...
int main(){
	...
	complex QamSymbs[SymbLen];
	phasemap QamTable;
	
	CheckParam();														// Check the parameters correctness
	RandGen(InfoBytes);													// Random generation of source stream
	GetQamTable(&QamTable);												// Retrieve the mapping table between bits and PSK constellation
	Mapper(InfoBytes,QamSymbs,&QamTable);								// Convert input bits into symbols
	//WriteSymbolsCsv(QamSymbs,SymbLen,TX);								// Write ideal Tx symbols into .csv file
	ChanAWGN(QamSymbs,EbN0);											// AWGN channel simulation
	//WriteSymbolsCsv(QamSymbs,SymbLen,RX);								// Write noisy Rx symbols into .csv file
	HardDemapper(QamSymbs,HardDemBytes,&QamTable);						// Hard-demapping of the PSK symbols	
	SoftDemapper(QamSymbs,SoftDemLLRs,&QamTable);						// Soft-demapping of the PSK symbols

	PrintParam();
	//PrintTable(&QamTable);
	//PrintBytes(InfoBytes,ByteLen,TX);
	//PrintSymbs(QamSymbs,SymbLen,TX);
...
/**
 *	Function for getting the mapping table between bit sets and QAM symbols according
 *	to Gray Coding.
 **/
void GetQamTable( phasemap *IoTable ){
...
void CheckWrongBits( uint8_t *ByteStreamA, uint8_t *ByteStreamB ){
	...
	printf(" - ERRORS : %d out of %d bits -> BER = %1.2e\n",ErrCount,InLen,(double)ErrCount/InLen);
