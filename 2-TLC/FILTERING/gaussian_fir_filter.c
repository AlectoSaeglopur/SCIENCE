...
#define SymbLen 	(uint16_t) 15										// Symbol stream length
#define sps 		(uint8_t) 8											// Samples per symbol
#define span 		(uint8_t) 8											// Number of symbols for filter representation
#define beta 		(float) 0.5	   										// Bandwidth - symbol time (BT) product



/*** CONSTANTS AND GLOBAL VARIABLES ***/
#define SampLen 	(uint16_t) (sps*SymbLen)							// Sample stream length
#define Ntaps 		(uint16_t) (sps*span+1)								// Number of filter taps
#define FirDelay 	(uint16_t) (sps*span/2)								// Filter delay (always (Ntaps-1)/2 for FIR filters) [Sa]
