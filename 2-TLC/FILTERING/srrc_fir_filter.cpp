 ...
 *** Date: 03/01/2019
 *** Reference: [1] Wikipedia - https://en.wikipedia.org/wiki/Root-raised-cosine_filter
 ***********************************************************************************************/
...
void GetSrrcTaps( float *Taps ){
	...
	float Time;
	float epsilon = 1e-30;
	float ZeroVal = 1/(2*pi*sps)*(pi*(beta+1)*sin(pi*(beta+1)/(4*beta))-4*beta*
      			sin(pi*(beta-1)/(4*beta))+pi*(beta-1)*cos(pi*(beta-1)/(4*beta)));

	for ( j=0; j<Ntaps; j++ ){
		Time = (j-(float)sps*span/2)/sps;
		if ( fabs(fabs(4*beta*Time)-1) < epsilon ){					// If zero-denominator index
			Taps[j] = ZeroVal;
		} else {
			Taps[j] = -4*beta/sps*(cos((1+beta)*pi*Time)+sin((1-beta)*pi*Time)/
				(4*beta*Time))/(pi*(pow(4*beta*Time,2)-1));
		}

	}

	Taps[sps*span/2] = -1/(pi*sps)*(pi*(beta-1)-4*beta);			// Set mid-point value
	ScaleSrrcTaps(Taps);
