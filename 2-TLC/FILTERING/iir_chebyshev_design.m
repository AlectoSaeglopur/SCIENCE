...
%%% Title: TYPE-I CHEBYSHEV IIR FILTER DESIGN
...
Fedge =[1 1.5 2.5 3]*1e3;                                           % Passband and stopband edges array [Hz] (see NOTE#1)
Att = [0.1 -60];                                                    % 2-element array specifying maximum passband ripple and minimum bandstop attenuation [dB]
...
[B,A,Order] = IirCheb(Fedge,Att,Ftype,Fs);                          % Design IIR Chebyshev taps                            
...
% >> Function to calculate IIR filter taps based on Type-I Chebyshev prototypes.
function [B,A,Order] = IirCheb( Fedge, Att, Ftype, Fs )
    ...
    switch Ftype
        case 'LPF'
            Vs = W(2)/W(1);
            n = ceil(acosh(sqrt((10^(-Att(2)/10)-1)/E2))/acosh(Vs));            % Estimate required filter order to satisty requirements
            [Num,Den] = GetChebCoef(n,Att(1));                                  % Get Chebyshev coefficients
            ...
            Order = n;                                                          % Calculate filter order (Ntaps = Order+1)
        case 'HPF'
            Vs = W(2)/W(1);
            n = ceil(acosh(sqrt((10^(-Att(2)/10)-1)/E2))/acosh(Vs));
            [Num,Den] = GetChebCoef(n,Att(1));
...
% >> Function to calculate the Chebyshev prototype coefficients specifying order and ripple.
%    Note the numerator and denominator polynomial arrays are sorted from higher to lower grade
%    (i.e. from gN*s^N to g0*S^0=g0).
function [ PolyNum, PolyDen ] = GetChebCoef( Ntaps, RipdB )
  Eps = sqrt(10^(RipdB/10)-1);
  beta = asinh(1/Eps)/Ntaps;
  if mod(Ntaps,2) == 0                                                          % if N is even...
    PolyNum = 1/sqrt(1+Eps^2);
    PolyDen = 1;
    for j = 0:Ntaps/2-1
      ak = (2*j+1)*pi/(2*Ntaps);
      bk = 2*sin(ak)*sinh(beta);
      ck = (sin(ak)*sinh(beta))^2+(cos(ak)*cosh(beta))^2;
      IterPoly = [1 bk ck];
...
% 1. "Fedge" represents the passband and stopband edges array containg the frequency edges in Hz for the transition-bands.
...
% 2. For some passband ripple values (Att(1)) it might happen that the designed transfer function is attenuated (i.e. max value < 0dB). This can be easily
%    recovered by multipling the "B" taps array by a corresponding rescaling factor.

% 3. MATLAB provides x2 built-in functions to design Type-I and Type-II Chebyshev filters (see examples below).

% Fs = 8e3;                                                   % Sample rate [Sa/s]                                
% Order = 10;                                                 % Filter order
% Fcut =  [1e3 1.5e3];                                        % Cut-off frequency [Hz]
% Rip = 0.5;                                                  % Passband ripple [dB]
% [B,A] = cheby1(Order,Rip,Fcut/(Fs/2),'bandpass');           % Design Type-I Chebyshev IIR filter taps

% Fs = 8e3;                                                   % Sample rate [Sa/s]                                
% Order = 10;                                                 % Filter order
% Fcut =  1e3;                                                % Cut-off frequency [Hz]
% Rip = 25;                                                   % Stopband ripple [dB]
% [B,A] = cheby2(Order,Rip,Fcut/(Fs/2),'low');                % Design Type-II Chebyshev IIR filter taps
