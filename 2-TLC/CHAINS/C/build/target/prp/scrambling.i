...
error_t Scramb_ListParameters( scr_par_t * ioParams )
{
  ...
  if (
# 52 "src\\scrambling.c" 3 4
     ((void *)0) 
# 52 "src\\scrambling.c"
          != ioParams)
  {
    ioParams->type = SCRAMB_ADT;

    if (SCRAMB_ADT == ioParams->type)
    {
      ioParams->nCells = 15u;
      ioParams->conVect = 0x00060000;
      ioParams->initSt = 0x95000000;
    }
    else if (SCRAMB_MLT == ioParams->type)
    {
      ioParams->nCells = 17u;
      ioParams->conVect = 0x00108000;
      ioParams->initSt = 0x2C350000;
    }
    else
    {
...
error_t Scramb_Descrambler( const byte_stream_t * inStream, byte_stream_t * outStream, const scr_par_t * pParams )
{
  ...
  if (Memory_IsStreamValid(inStream,inStream->id) &&
      Memory_IsStreamValid(outStream,outStream->id) &&
      (
# 138 "src\\scrambling.c" 3 4
      ((void *)0) 
# 138 "src\\scrambling.c"
           != pParams))
  {
    switch (pParams->type)
    {
...
static error_t Scramb_AdditiveScrambler( const byte_stream_t * inStream, byte_stream_t * outStream, const scr_par_t * pParams )
{
  ...
  if (Memory_IsStreamValid(inStream,inStream->id) &&
      Memory_IsStreamValid(outStream,outStream->id) &&
      (
# 187 "src\\scrambling.c" 3 4
      ((void *)0) 
# 187 "src\\scrambling.c"
           != pParams))
  {
    if (inStream->len == outStream->len)
    {
      ...
      for (j=0; j<bitLen; j++)
      {
        ...
        scrBit = ComputeRegBit(lfsrSt,pParams);
        lfsrSt >>= 1;

        if (1 == scrBit)
        {
          lfsrSt |= ((uint32_t) 0x80000000);
          scrBit ^= (outStream->pBuf[byteIdx]>>bitIdx) & ((uint8_t) 0x01);
...
static error_t Scramb_MultiplicativeScrambler( const byte_stream_t * inStream, byte_stream_t * outStream, const scr_par_t * pParams )
{
  ...
  if (Memory_IsStreamValid(inStream,inStream->id) &&
      Memory_IsStreamValid(outStream,outStream->id) &&
      (
# 282 "src\\scrambling.c" 3 4
      ((void *)0) 
# 282 "src\\scrambling.c"
           != pParams))
  {
    if (inStream->len == outStream->len)
    {
      ...
      for (j=0; j<bitLen; j++)
      {
        byteIdx = ((j)>>3u);
        bitIdx = (uint8_t)((8u -1)-(j&((uint32_t) 0x00000007)));
        inBit = ((inStream->pBuf[byteIdx]>>bitIdx)&((uint8_t) 0x01));
...
static error_t Scramb_MultiplicativeDescrambler( const byte_stream_t * inStream, byte_stream_t * outStream, const scr_par_t * pParams )
{
  ...
  if (Memory_IsStreamValid(inStream,inStream->id) &&
      Memory_IsStreamValid(outStream,outStream->id) &&
      (
# 345 "src\\scrambling.c" 3 4
      ((void *)0) 
# 345 "src\\scrambling.c"
           != pParams))
  {
    if (inStream->len == outStream->len)
    {
...
static uint8_t ComputeRegBit( uint32_t curSt, const scr_par_t * pParams )
{
  ...
  {
    if (((curSt<<j)&((uint32_t) 0x80000000)) & ((pParams->conVect<<j)&((uint32_t) 0x80000000)))
    {
      outBit ^= ((uint8_t) 0x01);
    }
  }
