...
FsRF = FcRF*100;                                                    % RF sample-rate [Sa/s] (NB: keep it high, the RF signal shall simulate the original analog one!)
...
function [ SgnOut, Time, FsUS ] = BandpassUndersamp( SgnIn, B, Fc, FsRF )
    X = 2;                                                          % BB oversampling factor (for better undesampled BB signal resolution)
    Fact = Fc/(X*B);
    if Fact ~= round(Fact)
